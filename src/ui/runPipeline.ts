import { checkRequiredColumns, requiredColumns } from "@core/constraints";
import { toCsv } from "@core/csv";
import { entriesToCsv, toRecords } from "@core/dataEntry";
import type { Pipeline } from "@core/pipeline";
import { literalCsv, planRun, type RunTask } from "@core/runner";
import type { Dag } from "@core/schema";
import type { NodeResult } from "@core/status";
import { forgetTable, loadCsv, queryRows, quote, tableColumns } from "./duckdb";
...
export async function runPipeline(
  ...
  for (const task of planRun(pipeline, dag, target)) {
    ...
    try {
      const rows = await materialize(task, results);
      const broken = await checkConstraints(task);
      if (broken) {
 ...
 */
async function checkConstraints(task: RunTask): Promise<string | undefined> {
  if (requiredColumns(task.node).length === 0) return undefined;
  const columns = await tableColumns(task.name);
...
async function materialize(
  ...
  switch (node.kind) {
    ...
    // run store alone.
    case "data_entry": {
      const source = await queryRows(`SELECT * FROM ${quote(node.input)}`);
      const csv = entriesToCsv(
        node,
        toRecords(source, node),
        result.entries ?? {},
      );
      return loadCsv(name, csv);
    }
...
async function createTable(name: string, query: string): Promise<number> {
  ...
  forgetTable(name);
  await queryRows(`CREATE OR REPLACE TABLE ${quote(name)} AS ${sql}`);
  const [row] = await queryRows(`SELECT count(*) AS n FROM ${quote(name)}`);
  return Number(row?.n ?? 0);
}

/** The first rows of a materialized table, for showing the run worked. */
export async function previewTable(name: string, limit = 5) {
  return queryRows(`SELECT * FROM ${quote(name)} LIMIT ${limit}`);
}
